...
def one_simulation(N, N_iter, dt, beta, lf, startfusion, sim, freediffusion, N_final, DT_Sim, DT_Sim_File):
	...
	ruido = np.random.uniform(-0.5*np.pi/N, 0.5*np.pi/N, N) 							# To randomize first-positions of particles
	firstpositions = np.linspace(0.0, 2*np.pi - 2*np.pi/N, N) + ruido 		# First positions of particles
	
	particles = [rfh.particle(q, rfh.wrapAngle(firstpositions[k])) for k in range(N)] # Associate to each charge q its position in the circle
	particles = sorted(particles, key = attrgetter('position')) 											# Organize all particle positions
	
	t_col = 't_' + str(sim)
	N_col = 'N_' + str(sim)
	data_n = pd.DataFrame([[t, len(particles)]], columns = [t_col, N_col]) # For time-evolution of Total number of particles N=N(t)
	hist = np.histogram([each_particle.charge for each_particle in particles], np.arange(q/2, q*(N+3/2), q)) # Frequency data for charges
	hists = [[sim, t, hist]] # For time-evolution of frequency charges data
	...
	while k < N_iter and len(particles) > N_final:
		...
		rfh.stepup(particles, normalnoise, freediffusion, dt/f) # Use the Langevin Eq. to find delta theta for each particle
		DT_Sim['step'] = pd.Series([particles[steps].step for steps in range(len(particles))]) # Add delta theta to DT_Sim matrix
		
		# Move all particles by step
		for each_particle in particles:
			each_particle.update_position()
		
		DT_Sim['newpos'] = pd.Series([particles[positions].position for positions in range(len(particles))]) # Old-postition to New-position of the particles to make evolve the system
		
		#print(DT_Sim, file = DT_Sim_File) # Print data into the file: Sim_Data.dat
		if k == 0:
